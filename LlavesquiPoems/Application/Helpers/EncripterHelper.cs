...
public class EncripterHelper
{
    ...
    public EncripterHelper(EncodeConfiguration encodeConfig)
    {
    ...
    {
        byte[] Key = Encoding.ASCII.GetBytes(_encodeConfig.key + _encodeConfig.key);
        //aes.Key = Encoding.ASCII.GetBytes(key);
        byte[] IV = Encoding.ASCII.GetBytes(_encodeConfig.ivt);
        // Check arguments.
    ...
    public string DecryptStringFromBytes_Ae(string token, byte[] Key, byte[] IV)
    {
        // Check arguments.
        ...
        byte[] cipherText = JsonSerializer.Deserialize<byte[]>(token);
        // Create an Aes object
        // with the specified key and IV.
        using (System.Security.Cryptography.Aes aesAlg = System.Security.Cryptography.Aes.Create())
        {
